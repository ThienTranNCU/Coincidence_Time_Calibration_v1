...
//		cout<<"Killl                               ************************         "<<chi2<<" "<<kill<<endl;
    ...
    inD.close();
         	...
         	///////////////////////////////////////////////////////////// Time Calibration ///////////////////////////////////////////////////////////////////////////////////
    sprintf(argCal,"%s",argv[8]);   
    inD.open(Form(argCal));
    
    Double_t CTW[16][16][7][2]={0};
    Int_t pt1,pt2;
 ...
 //   for(Int_t i=0;i<1024;i++){
	for(Int_t i=0;i<16;i++){
		for(Int_t j=0;j<16;j++){
			inD>>pt1>>pt2;
			for(Int_t p=0;p<7;p++){
				inD>>CTW[pt1][pt2][p][0]>>CTW[pt1][pt2][p][1];
				if(CTW[pt1][pt2][p][1]>20.0){
					CTW[pt1][pt2][p][1]=0;
					CTW[pt1][pt2][p][0]=0;
				}
			}
    ...
    sprintf(newFolder,"%s/%s_Processed3",argv[1], argv[2]);                                ////////////////////////////////////////////////////Creating a new folder for the output
...
//                                   )
                                       ...
                                       )
                                {
                                    ...
                                    {   cluster=0;
                                        ...
                                        ///////////////////////////////////Adopting timing calibration
                                        
                                        if((cm[i1][8]==0)||(cm[i1][8]==1)){
												pt1=int(int(x1/4)+8*int(y1/8));
												pt2=int(int(x2/4)+8*int(y2/8));
										}else{
												pt1=int(int(x2/4)+8*int(y2/8));
												pt2=int(int(x1/4)+8*int(y1/8));
										}
                                        ////////////////////////////////////////////////
                                        
                                        
                                      //  if((cm[i1][7]-cm[j1][7])<300&&((cm[i1][7]-cm[j1][7])>-300))//FULL COINCIDENCE BAND:-300to300
                                      //7 point maximum time window condition
                                            if((((cm[i1][7]-cm[j1][7])<(CTW[pt1][pt2][0][0]+3*CTW[pt1][pt2][0][1]))&&((cm[i1][7]-cm[j1][7])>(CTW[pt1][pt2][0][0]-3*CTW[pt1][pt2][0][1])))||(((cm[i1][7]-cm[j1][7])<(CTW[pt1][pt2][1][0]+3*CTW[pt1][pt2][1][1]))&&((cm[i1][7]-cm[j1][7])>(CTW[pt1][pt2][1][0]-3*CTW[pt1][pt2][1][1])))||(((cm[i1][7]-cm[j1][7])<(CTW[pt1][pt2][2][0]+3*CTW[pt1][pt2][2][1]))&&((cm[i1][7]-cm[j1][7])>(CTW[pt1][pt2][2][0]-3*CTW[pt1][pt2][2][1])))||(((cm[i1][7]-cm[j1][7])<(CTW[pt1][pt2][3][0]+3*CTW[pt1][pt2][3][1]))&&((cm[i1][7]-cm[j1][7])>(CTW[pt1][pt2][3][0]-3*CTW[pt1][pt2][3][1])))||(((cm[i1][7]-cm[j1][7])<(CTW[pt1][pt2][4][0]+3*CTW[pt1][pt2][4][1]))&&((cm[i1][7]-cm[j1][7])>(CTW[pt1][pt2][4][0]-3*CTW[pt1][pt2][4][1])))||(((cm[i1][7]-cm[j1][7])<(CTW[pt1][pt2][5][0]+3*CTW[pt1][pt2][5][1]))&&((cm[i1][7]-cm[j1][7])>(CTW[pt1][pt2][5][0]-3*CTW[pt1][pt2][5][1])))||(((cm[i1][7]-cm[j1][7])<(CTW[pt1][pt2][6][0]+3*CTW[pt1][pt2][6][1]))&&((cm[i1][7]-cm[j1][7])>(CTW[pt1][pt2][6][0]-3*CTW[pt1][pt2][6][1]))))
                                        {
...
//        c2Mapt->SaveAs(h2MaptTitle);
   ...
   ofstream out;
   sprintf(pdfname,"%s/LOR_b-x-y.txt",newFolder);
	out.open(Form(pdfname));
   
   
   for(Int_t i1=0;i1<32;i1++){
	   for(Int_t j1=0;j1<16;j1++){
			out<<0<<" "<<i1<<" "<<j1<<" "<<hLOR1->GetBinContent(i1+1,j1+1)<<endl;
	   }	   
	}
   for(Int_t i1=0;i1<32;i1++){
	   for(Int_t j1=0;j1<16;j1++){
			out<<1<<" "<<i1<<" "<<j1<<" "<<hLOR2->GetBinContent(i1+1,j1+1)<<endl;
	   }	   
	}
   out.close();
   
      ofstream out1;
   sprintf(pdfname,"%s/Single_b-x-y.txt",newFolder);
	out1.open(Form(pdfname));
   
   
   for(Int_t i1=0;i1<32;i1++){
	   for(Int_t j1=0;j1<16;j1++){
			out1<<0<<" "<<i1<<" "<<j1<<" "<<hSingle0->GetBinContent(i1+1,j1+1)<<endl;
	   }	   
	}
   for(Int_t i1=0;i1<32;i1++){
	   for(Int_t j1=0;j1<16;j1++){
			out1<<1<<" "<<i1<<" "<<j1<<" "<<hSingle1->GetBinContent(i1+1,j1+1)<<endl;
	   }	   
	}
   out1.close();
